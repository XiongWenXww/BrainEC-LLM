...
from utils.postprocess import softThres, change01GPU
...
class EarlyStopping: # loss越小越好，当大于patience次的loss没超过目前最好的loss时，停止退出
    ...
    # def __call__(self, val_loss, model, path):
        ...
        if self.best_score is None:
            ...
            if self.accelerator is None:
                print(f'EarlyStopping counter: {self.counter} out of {self.patience}, best loss: {self.best_score}')
            else:
...
def matrix_to_text(matrix):
    N = matrix.shape[0]
    text = f'The following is a matrix of Pearson correlation coefficients for {N} fMRI time series, with the number i denoting the i-th brain region:\n'
    # labels = ["1", "2", "3", "4", "5"]
    labels = [str(i) for i in range(1, N+1)]
    text += ", ".join(labels) + "\n"
    for label, row in zip(labels, matrix):
        formatted_row = ", ".join(f"{val:.2f}" for val in row)
        text += label + ": " + formatted_row + "\n"
    return text

def adj_description(adj, lower_bound, upper_bound, soft_threshold=0.5):
    adj = adj - torch.diag(torch.diag(adj)) # set the diagonal values to 0
    threshold = softThres(adj, soft_threshold=soft_threshold)
    adj_binary = change01GPU(adj, threshold=threshold)
    print('threshold:', threshold, 'adj_binary:\n', adj_binary)
    ...
    for i in range(N):
        for j in range(i + 1, N):
            if adj_binary[i, j] == 1:
                ...
                description += f"Brain region {i+1} and brain region {j+1} have a {connection_strength} connection.\n"
            else:
                description += f"Brain region {i+1} and brain region {j+1} have no connection.\n"
